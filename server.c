 ...
 * forms a list of players by appending them into the obj parameters*/
...
int main(int argc, char *argv[])
{
		...
		/*Server's main loop - wait for requests and handle 'em properly*/
accept_req:
		while (1) {
				if ((conn_fd = accept(listen_fd, (struct sockaddr*) &cli_addr, &addr_len)) < 0) {
				...
				pthread_create(&thr_id, NULL, &server_thread, (void*) &conn_fd);
		}
...
void *server_thread(void *par)
{
		...
		if something's wrong, check whether that casting is needed*/
		...
		while ((len = recv(thread_fd, data_buf, MAX_RCV_LEN, 0))) {
				...
				/*"join" query*/
				/*TODO: think of a better solution of checking 2nd argument*/
		...
		/*When clients closes connection, remove entry from hashmap, close the socket's file descriptor
		* and thread*/
		pthread_mutex_lock(&sync_proc);
		if (strlen(player_name) > 1) {
				sm_get(clients, player_name, data_buf, sizeof(data_buf));
				if (strcmp(data_buf, addr_buf) == 0) {
						/*This entry is not fully removed; key is kept, only value is deleted*/
